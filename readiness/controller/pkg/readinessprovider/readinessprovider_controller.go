...
type ReadinessProviderReconciler struct {
	...
	PodExecutionCondition      func(context.Context, string, string, *corev1alpha2.PodExecutionCondition) (corev1alpha2.ReadinessConditionState, string)
}
...
func (r *ReadinessProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for i, condition := range readinessProvider.Spec.Conditions {
		...
		if condition.ResourceExistenceCondition != nil {
			...
			state, message = r.PodExecutionCondition(ctxCancel, readinessProvider.Name, condition.Name, condition.PodExecutionCondition)
		}
